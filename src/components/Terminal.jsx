...
// Inject styles (avoid duplicate injection)
if (typeof document !== 'undefined' && !document.querySelector('#terminal-styles')) {
...
// Global store for terminal sessions to persist across tab switches
...
function Terminal({ selectedProject, isActive }) {
  ...
  // Disconnect from terminal function
  ...
  const restartTerminal = () => {
    ...
    const sessionKeys = Array.from(terminalSessions.keys()).filter(key => 
      key.includes(selectedProject.name)
    );
    sessionKeys.forEach(key => terminalSessions.delete(key));
  ...
  useEffect(() => {
    ...
    const sessionKey = `terminal-${selectedProject.name}`;
    ...
    terminal.current = new XTerm({
      ...
      theme: {
        ...
        // Extended colors for better output
        extendedAnsi: [
  ...
  const connectWebSocket = async () => {
    ...
    try {
      ...
      // Create WebSocket URL for plain terminal (different path)
      const wsUrl = `${wsBaseUrl}/terminal`;
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center text-gray-500 dark:text-gray-400">
          ...
          <p>Choose a project to open a terminal in that directory</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-shrink-0 bg-gray-800 border-b border-gray-700 px-4 py-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-gray-400">Plain Terminal</span>
            {!isInitialized && (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-90 p-4">
            <div className="text-center max-w-sm w-full">
              <button
                ...
                className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 text-base font-medium w-full sm:w-auto"
              ...
              <p className="text-gray-400 text-sm mt-3 px-2">
                Start a plain shell session in {selectedProject.displayName}
              </p>
            </div>
          </div>
        )}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-90 p-4">
            <div className="text-center max-w-sm w-full">
              <div className="flex items-center justify-center space-x-3 text-blue-400">
                <div className="w-6 h-6 animate-spin rounded-full border-2 border-blue-400 border-t-transparent"></div>
